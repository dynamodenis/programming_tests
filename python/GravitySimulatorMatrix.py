...
def solution(board: List[List[str]]):
    ...
    for j in range(cols):
        ...
        print(f"coumn elements {column_elements}")
        current_idx_in_elements = len(column_elements) -1 #for us to start from the last elemnt
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if board_after_fall[i][j] == "*" and i > 0 and board_after_fall[i-1][j] == "#":
    ...
    for exp_r, exp_c in explosion_centers:
        ...
        cells_to_destroy.add((exp_r, exp_c))  #This part here destroys the obstacle but we should not destroy the obstacle
